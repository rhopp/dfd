...
5. **Phase 2.7 — Analysis Validation**: Validates every `analysis.md` against the expected machine-parseable format. Checks all required fields (Root Cause, Category, Failed Task, Failed Step, Evidence section, Details section) and validates root_cause against the component's taxonomy. Invalid analyses are re-run with specific feedback about what's wrong (up to 2 retries). Any remaining invalid analyses are replaced with a complete fallback template (root_cause `unknown`, metadata from `metadata.json`), guaranteeing clean data for downstream consumers.
