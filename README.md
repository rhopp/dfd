...
5. **Phase 2.7 — Root Cause Validation**: Validates every analysis against the component's taxonomy. If an agent produced an invalid root_cause (not in the valid set), the analysis is re-run in a loop (up to 2 retries). Any remaining invalid values are forced to `unknown` as a fallback, guaranteeing clean data for downstream consumers.
